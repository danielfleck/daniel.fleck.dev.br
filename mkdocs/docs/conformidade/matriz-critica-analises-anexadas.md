...
## Achados adicionais de 15/08/2026

| Tema | Tratamento | Razão |
|---|---|---|
| HTTP não redireciona corretamente para HTTPS | Aceito | repositório-base não contém regra de rewrite; implementar conforme camada Apache/Varnish confirmada |
| HSTS ausente | Aceito | repositório-base não contém `Strict-Transport-Security`; implementar em duas fases |
| `max-age` de um ano | Aceito como objetivo técnico da auditoria | RFC 6797 define a política e dá um ano como exemplo, mas não transforma esse valor em mínimo legal geral |
| validar todos os subdomínios | Aceito somente se houver `includeSubDomains`/preload | nesta release não haverá essas diretivas |
| `security.txt` ausente | Aceito como boa prática faltante | RFC 9116 define o formato e a localização; não é classificado como vulnerabilidade legal por si só |
| HTML/contraste/console/touch targets | Pendente | relatório resume o problema, mas faltam seletores/linhas concretas; repetir medição após a release |
| `llms.txt` ausente | Não corrigir | o próprio relatório afirma que é proposta comunitária e ausência não é erro |
